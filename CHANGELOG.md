...
## 1.4.2 - 2025-11-17
### Fixed
- This package will no longer throw an error when it tries to release a savepoint that doesn't exist
  - This should prevent most `ERROR: savepoint "..." does not exist` errors

## 1.4.1 - 2025-11-13
### Fixed
- Using `.then(...)` without a second (`reject`) parameter on a Promise returned by the proxy Prisma Client no longer throws a `reject is not a function` error (#20)
